...
export default function Note({ note, dispatch }) {
  ...
  return (
    ...
    >
      ...
      <div className="title-container" style={{ color: `${parseInt(Number(parseInt(note.timestamp.replace(/^#/, ''), 16)), 10) < parseInt(Number("0x808080"), 10) ? 'white' : 'black'}` }}>{note.title}</div>
      <div
